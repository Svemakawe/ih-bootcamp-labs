...
function turnHoursToMinutes(array) {
  ...
  array.forEach(function(item) {
    if (item.duration.length === 7) {
      item.duration = parseInt(item.duration.substring(0, 1)) * 60 + parseInt(item.duration.substring(3, 5));
      newArray.push(item);
    } else if (item.duration.length === 6) {
      item.duration = parseInt(item.duration.substring(0, 1)) * 60 + parseInt(item.duration.substring(3, 4));
      newArray.push(item);
    } else {
      item.duration = parseInt(item.duration.substring(0, 1)) * 60;
      newArray.push(item);
    } 
  })
...
function dramaMoviesRate(array) {
  let dramaArr = array.filter(function(item){
